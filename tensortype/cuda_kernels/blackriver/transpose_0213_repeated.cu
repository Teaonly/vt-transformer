...
template <>
int transpose_0213_repeated<float>(const float* src, float* target, int a, int b, int cf, int ct, int d, cudaStream_t stream) {
...
template <>
int transpose_0213_repeated<__half>(const __half* src, __half* target, int a, int b, int cf, int ct, int d, cudaStream_t stream) {
    int nElementNumber = a * b * ct * d;
